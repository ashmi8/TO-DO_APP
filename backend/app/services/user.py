...
from app.schemas.user import UserInputSchema, UserOutputSchema, ListUsersSchema
...
async def get_all_users_data() -> ListUsersSchema:
    ...
    return ListUsersSchema(users=users_list)
...
async def update_user_by_id(
    user_id: PydanticObjectId, user_data: UserInputSchema
) -> UserOutputSchema:
    existing_user = await User.find_one(User.id == user_id)
    if not existing_user:
        raise HTTPException(status_code=404, detail="User not found")
